...
import { clientes as iniciales, type Cliente } from "@/lib/mock";
import { PageTitle, Card, FilterSelect, SearchBox } from "@/components/ui";
import { Modal, Field, Input, Select } from "@/components/modal";

type Row = Cliente & { _id: string };
const vacio = { nombre: "", tipo: "Física", telefono: "", email: "" };

export default function ClientesPage() {
  const [lista, setLista] = useState<Row[]>(iniciales.map((c, i) => ({ ...c, _id: `c${i}` })));
  ...
  function abrirEditar(c: Row) {
    setEditId(c._id);
    setForm({ nombre: c.nombre, tipo: c.tipo, telefono: c.telefono, email: "" });
    setOpen(true);
  }
  function borrar(id: string) {
    if (confirm("¿Eliminar este cliente?")) setLista((l) => l.filter((c) => c._id !== id));
  }
  function guardar() {
    const tipo = form.tipo === "Moral" ? "Moral" : "Física";
    if (editId) {
      setLista((l) => l.map((c) => (c._id === editId ? { ...c, nombre: form.nombre, tipo, telefono: form.telefono || "—" } : c)));
    } else {
      setLista((l) => [{ _id: `c${Date.now()}`, nombre: form.nombre, tipo, telefono: form.telefono || "—", expedientes: 0, ultimaAsesoria: null }, ...l]);
    }
    setOpen(false);
  }
  ...
  return (
    <>
              ...
              <tr key={c._id} className="hover:bg-paper/60 transition-colors">
                ...
                <td className="px-3 py-3.5">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => abrirEditar(c)} title="Editar" className="p-1.5 rounded-md text-muted hover:text-navy hover:bg-navy/[.06] transition-colors"><Pencil size={16} /></button>
                    <button onClick={() => borrar(c._id)} title="Eliminar" className="p-1.5 rounded-md text-muted hover:text-danger hover:bg-danger-wash transition-colors"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editId ? "Editar cliente" : "Nuevo cliente"} onSubmit={guardar} submitLabel={editId ? "Guardar cambios" : "Crear cliente"}>
        <Field label="Nombre" full>
          <Input value={form.nombre} onChange={(e) => set("nombre", e.target.value)} placeholder="Nombre o razón social" required />
        </Field>
        <Field label="Tipo">
          <Select options={["Física", "Moral"]} value={form.tipo} onChange={(e) => set("tipo", e.target.value)} />
        </Field>
